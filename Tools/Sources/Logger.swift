...
//  Chatter
//
//  Created by Kirill Khlopko on 2/6/17.
...
public final class Logger {
    ...
    private func printMessage(params: Parameter...,
                              separator: String, file: String, function: String, line: Int, level: Level) {
...
public final class Calculagraph {

    private var timePoints: [(description: String, absoluteTime: CFAbsoluteTime)] = []
    private var measureResults: [String: CFAbsoluteTime] = [:]
    private var startTime: CFAbsoluteTime = 0.0
    private static let instance = Calculagraph()
    ...
    public static func start() {
        instance.timePoints = []
        instance.startTime = CFAbsoluteTimeGetCurrent()
        instance.measureResults = [:]
    }

    public static func addPoint(name: String) {
        let time = CFAbsoluteTimeGetCurrent()
        instance.timePoints.append((name, time))
    }

    public static func print(endPoint: String) {
        addPoint(name: endPoint)
        var previous = instance.startTime
        var output: String = ""
        for (name, time) in instance.timePoints {
            let diffFromPrev = (time - previous) * 1000
            let diffFromStart = (time - instance.startTime) * 1000
            output += String(format: "\(name) | from start = %4.2lf | prev = %4.2lf\n", diffFromStart, diffFromPrev)
            previous = time
        }
        Swift.print(String(repeating: "_", count: 20))
        Swift.print(output)
        Swift.print(String(repeating: "_", count: 20))
    }

    public static func measure(withName name: String, closure: () -> ()) {
        let start = CFAbsoluteTimeGetCurrent()
        closure()
        let diff = CFAbsoluteTimeGetCurrent() - start
        let result = (instance.measureResults[name] ?? 0) + diff
        instance.measureResults[name] = result
    }

    public static func printMeasureResults() {
        var output: String = ""
        for (name, time) in instance.measureResults {
            output += String(format: "measure \(name) | time = %4.1lf\n", time * 1000)
        }
        Swift.print(String(repeating: "_", count: 20))
        Swift.print(output)
        Swift.print(String(repeating: "_", count: 20))
